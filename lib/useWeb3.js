...
export default function useWeb3() {
    ...
    const approval = async () => {
        const isApproved = await fnftContract.methods.isApprovedForAll(account, marketplaceAddress).call();
        if (isApproved) {
            return;
        }
        const result = await fnftContract.methods.setApprovalForAll(marketplaceAddress, true).send({ from: account });
